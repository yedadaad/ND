...
namespace TarodevController
{
    ...
    public class PlayerController : MonoBehaviour, IPlayerController
    {
            ...
            switch (playZT)
            {
                ...
                if(IE==true)
                {
                    ...
                    if (Input.Y != 0)
                    {
                        ...
                        // Apply bonus at the apex of a jump
                        var apexBonus = Mathf.Sign(Input.Y) * _apexBonus * _apexPoint;
                        _currentVerticalSpeed += apexBonus * Time.deltaTime;
                    }
                    else
                    {
                        // No input. Let's slow the character down
                ...
                case play.出海:
                    ...
                    else
                    {
                        ...
                        // Fall
                        _currentVerticalSpeed -= (fallSpeed * Time.deltaTime) / XialuoSpeed;
                        XialuoSpeed = Mathf.MoveTowards(XialuoSpeed, 1, _deAcceleration * Time.deltaTime);
        ...
        IEnumerator Shuihua()
        {
            IE = true;
            Debug.Log("1");
            GameObject sh = Instantiate(shuihua);
            sh.transform.position = transform.position+new Vector3(0,-0.5f,0);
